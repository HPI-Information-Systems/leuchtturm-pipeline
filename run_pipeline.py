...
def run_email_pipeline(conf):
    ...
    reader = EmlReader(conf.get('data', 'source_dir'))
    pipes = [
    ...
    writer = TextFileWriter(path=conf.get('data', 'working_dir'))
    Pipeline(reader, pipes, writer).run()

    if conf.get('topic_modelling', 'train_model'):
        run_topic_model_training(conf)

    reader = TextFileReader(path=conf.get('data', 'working_dir'))
    writer = TextFileWriter(path=conf.get('topic_modelling', 'working_dir'))
    ...
    if conf.get('solr', 'import'):
        ...
        SolrFileWriter(conf.get('topic_modelling', 'working_dir'), conf.get('solr', 'topic_collection')).run()
...
def run_topic_model_training(conf):
    ...
    df = EmlReader(conf.get('data', 'working_dir')).run()
