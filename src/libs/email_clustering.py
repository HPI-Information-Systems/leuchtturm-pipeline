...
class Evaluation:
    ...
    def extract_cluster_information(df, clusters, subject_vect, body_vect):
        top_subject_features_per_cluster = Evaluation.top_feats_per_cluster(
            ...
            clusters,
            subject_vect.get_feature_names()
        )

        cluster_information_per_cluster = []

        for idx, cluster in enumerate(top_subject_features_per_cluster):
            cluster = cluster.loc[cluster['score'] != 0, :]
            cluster = cluster.reset_index(drop=True)
            features = cluster['features'].values
            if len(features) > 10:
                features = features[:10]
            cluster_information_per_cluster.append({
                'cluster': idx,
                'top_subject_words': features
            })

        top_body_features_per_cluster = Evaluation.top_feats_per_cluster(
            df[[col for col in df.columns if col.startswith('feat_body')]].values,
            clusters,
            body_vect.get_feature_names()
        )
        top_body_words_per_cluster = []
        for idx, cluster in enumerate(top_body_features_per_cluster):
            cluster = cluster.loc[cluster['score'] != 0, :]
            cluster = cluster.reset_index(drop=True)
            features = cluster['features'].values
            if len(features) > 10:
